...
- [x] **Bug fix — xref-driven changed-object set (hybrid-reference false positive)** (2026-06-15)
  - **Symptom:** `Microsoft-Sample-Invoice_clear.pdf` (clean hybrid-reference PDF
    using cross-reference streams) scored revision_recovery **MEDIUM(60)** off
    ~4183 phantom "new CONTENT objects" with no text change → fused **HIGH(85)**.
  - **Root cause:** the changed-object SET was built by diffing two pikepdf object
    *enumerations* from differently-truncated reparses. qpdf's enumeration of a
    truncated revision depends on what its xref can resolve, so the truncated
    first revision under-enumerated (~62 objects) vs 4244 in the full file →
    ~4183 phantom objects. The real final increment is a 184-byte compatibility
    xref append (empty `0 0` classic subsection + a back-pointing `/XRefStm`
    311774, < the increment start) that authors ZERO objects.
  - **Fix:** `diff/objectdiff.py` now derives the changed set from
    `objects_written_in_increment(raw, start, end)` — the in-use `(num, gen)`
    whose xref record is physically authored inside `rev_b.data[len(rev_a.data):]`.
    It reads the increment's own cross-reference: classic `xref` table (parsed in
    place; empty `0 0` → nothing), cross-reference *stream* (loaded via
    `pdf.get_object((num,gen)).read_bytes()`, decoded by `/W`+`/Index`, type-1 &
    type-2), and any `/XRefStm` **only** when the referenced offset lies within
    `[start, end)` (a back-pointer authors nothing → the compatibility case is
    zero writes with no special-casing). `diff_objects` also guards that
    `rev_a.data` is a true byte-prefix of `rev_b.data` (always true in the
    pipeline); a non-incremental pair yields no changes. NO cross-revision
    enumeration diff remains anywhere. The reconstruct-by-truncation + pdfminer
    text-diff path is unchanged.
  - **Classifier:** `/Type /XRef` and `/Type /ObjStm` containers now → META
    (structural plumbing), never CONTENT.
  - **Tests:** `tests/test_objects_written.py` (classic-table / xref-stream /
    hybrid-compatibility increments + defensive contract);
    `tests/test_objectdiff.py` migrated to genuine incremental updates
    (`_append_increment` appends new objects + classic xref + `/Prev` trailer, so
    rev_a is a real byte-prefix of rev_b); acceptance guard
    `test_microsoft_hybrid_revision_recovery_not_medium`. Regression guard intact:
    `Acrobat_Demo_File.pdf` xref-stream tamper still HIGH(95); genuine
    incremental edit still CONTENT→HIGH. Full suite: **444 passed, 1 skipped.**
