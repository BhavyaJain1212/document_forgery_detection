...
6.1 (this slice): ``aggregate()``, the PHI-scrub boundary, prompt assembly, and
``StubAdvisoryEngine`` are implemented and CPU-only. 6.2 (not yet implemented):
``LocalLLMAdvisoryEngine``, the FastAPI handlers in ``api.py``, and pipeline
wiring. Full design: ``docs/STAGE6_DESIGN.md``.
"""
