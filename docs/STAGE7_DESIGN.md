...
**Status: DESIGN CONTRACT (Session 7.0). No logic implemented this session —
data models / config / interfaces + the advisory prompt are real; every logic
function is a `NotImplementedError` stub.** Implementation lands in 6.1 / 7.2
(see `docs/TODO.md`).

Stage 7 is **not a detector**. The five stages before it
...
headline. Stage 7 is the **assembly + presentation layer** that turns that
...
the full Stage 7 build come later.
...
- Subpackage: `src/pdf_forgery/aggregate/`, downstream of every detection stage.
  ...
  the reverse. (Doc named `STAGE7` ↔ package `aggregate`, mirroring the
...
- **Headline fusion is delegated, not reinvented.** `aggregate()` calls the
  ...
  deferred" means Stage 7 adds NO new fusion math this slice — it wraps the
...
stages, escalation bonus, bands). **Deferred to full Stage 7:** geometric
...
- [x] Stubs compile; **no logic implemented**; the aggregate layer is NOT yet run
      after the pipeline (wiring is a 7.2 task).
- [x] `docs/TODO.md` updated with the 6.1 / 7.2 breakdown.
</content>
</invoke>
